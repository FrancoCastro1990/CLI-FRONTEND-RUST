...
# CLI Frontend Generator - Linux Quick Install Script
...
DOWNLOAD_URL=$(echo "$LATEST_RELEASE" | grep -o "https://github.com/${REPO_OWNER}/${REPO_NAME}/releases/download/[^\"]*${BINARY_NAME}-linux-x86_64[^\"]*" | head -1)

if [ -z "$DOWNLOAD_URL" ]; then
    echo -e "${RED}❌ Precompiled binary not available for Linux x86_64${NC}"
    exit 1
fi
