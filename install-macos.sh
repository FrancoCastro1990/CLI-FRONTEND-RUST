...
# CLI Frontend Generator - macOS Quick Install Script
...
echo -e "${BLUE}=============================================${NC}"
echo -e "${BLUE}🏗️ Detected Architecture: ${ARCH}${NC}"
...
if [ -z "$DOWNLOAD_URL" ]; then
    echo -e "${RED}❌ Precompiled binary not available for macOS ${ARCH}${NC}"
    exit 1
fi
