...
2. **Template Introspection** - `--describe` command for exploring template details before generation
3. **Intelligent Name Processing** - Applies context-aware transformations based on file types and conventions
4. **Handlebars Integration** - Leverages powerful templating with conditional logic and custom helpers
5. **Multi-Convention Support** - Handles PascalCase, camelCase, snake_case, kebab-case transformations
6. **Architecture Integration** - Seamlessly integrates with 12 software architecture patterns
...
## 🔍 Exploring Templates with `--describe`

Before diving into template development or using a template, you can explore its complete configuration using the `--describe` command:

### Basic Usage

```bash
# View detailed information about any template
cli-frontend --describe component

# Explore hook template
cli-frontend --describe hook

# Check API service options
cli-frontend --describe api-service
```

### What `--describe` Shows

The command provides comprehensive information about a template:

1. **Template Metadata**
   - Name and description from `[metadata]` section
   - Author and version information

2. **Available Variables**
   - All variables defined in `[options]` section
   - Variable types (boolean, enum, string)
   - Possible values for enumerated variables
   - Default values
   - Descriptions

3. **File Generation Rules**
   - Files always generated (`always`)
   - Files generated by default (`default`)
   - Conditionally generated files with their conditions
   - Clear indication of when each file is created

4. **Usage Examples**
   - Basic usage with defaults
   - Examples with different variable combinations
   - Full-featured usage showing all options
...
### Benefits for Template Developers

When creating custom templates, use `--describe` to:

- ✅ **Verify Configuration** - Ensure `.conf` file is parsed correctly
- ✅ **Test Variable Metadata** - Confirm `_options`, `_type`, `_description` work as expected
- ✅ **Validate File Conditions** - Check that conditional file generation is configured properly
- ✅ **Document Usage** - Auto-generated examples serve as instant documentation

**Pro Tip**: After creating a new template, always run `--describe` to verify the configuration before actual code generation.
